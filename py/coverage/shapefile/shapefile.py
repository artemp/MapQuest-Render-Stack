...
class Reader:
	...
	def __shapeIndex(self, i=None):
		...
		if not self._offsets:
			...
			for r in range(numRecords):
				...
				( offset, length ) = unpack( ">2i", f.read(8) )
				self._offsets.append( offset * 2 )
	...
	def shapes(self):
		...
		if len( self._offsets ) > 0:
			...
			for shapeIndex in range( 0, len( self._offsets ) ):
				f.seek( self._offsets[ shapeIndex ] )
...
class Writer:
	...
	def __shapefileHeader(self, fileObj, headerType='shp'):
		...
		if self.shapeType != 0:
			...
			f.write(pack("<4d", 0.0,0.0,0.0,0,0))
    # Elevation
		z = self.zbox()
		# Measure
		m = self.mbox()
